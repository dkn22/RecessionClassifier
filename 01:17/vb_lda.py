...
# 1. Initialization
	def __init__(self, K, alpha, eta, optimize_alpha = True):
	...
	def fit(self, corpus, 
		...
		**kwargs):
		...
		if self.optimize_alpha:
			print('The algorithm will update the hyperparameter alpha using numerical optimization (%s).' %alpha_optimize_method)
		...
		while abs(elbo - previous_elbo) > convergence_thres and counter < max_iter+1:
			...
			if self.optimize_alpha and counter % alpha_update_interval == 0:
				if alpha_optimize_method == 'newton-raphson':
					self.update_alpha(alpha_convergence_threshold, alpha_max_iter)
				else:
...
# 2.3 Dirichlet hyperparameter optimisation
	...
	def update_alpha(self, convergence_threshold=1e-5, max_iter = 100):
		...
		for iterr in range(max_iter):
			...
			gradient = self.num_docs * (psi(alpha_sum) - psi(self.alpha)) + gradient_gamma_terms
			...
			while self.alpha - step_size * alpha_update < 0:
	...
	def optimize_alpha(self, optimize_method):
		...
		def alpha_gradient(alpha, gamma, num_docs):
		    ...
		    gradient_gamma_terms = psi(gamma) - psi(np.sum(gamma, axis = 1)).reshape(-1, 1)
		    gradient_gamma_terms = np.sum(gradient_gamma_terms, axis=0) # scalar

		    gradient = num_docs * (psi(alpha_sum) - psi(alpha)) + gradient_gamma_terms
		...
		alpha_optimal = optimize.root(alpha_gradient, alpha, jac = alpha_hessian, args=(gamma, num_docs), method = optimize_method)
